...
let
    @variables x
    y = 0.125 + 0.875 * (1 + (x/3000)^(1/0.3))^(-0.7)
    dy_dx = Symbolics.derivative(y, x; simplify=true)
    println(dy_dx)
end
∂S∂ψm(ψm) = (-5.243009636293373e-12(ψm^2.3333333333333335)) / ((1 + 2.5680047198171624e-12(ψm^3.3333333333333335))^1.7)
Sr(ψm) = 0.125 + 0.875 * (1 + (ψm/3000)^(1/0.3))^(-0.7)
kr(S) = sqrt(S) * (1 - (1 - S^(1/0.7))^0.7)^2
...
function tprocedure!(conf::Config, grid::DeviceGrid{T1, T2}, mpts::DeviceParticle{T1, T2}, vpos, vcol) where {T1, T2}
    ...
    while t_cur < t_tol
        ...
        dev_mpts.ext.vw[dev_mpts.ext.bcp, 3] .= dev_mpts.ext.val
