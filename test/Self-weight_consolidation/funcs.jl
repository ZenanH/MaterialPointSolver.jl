...
@kernel function tp2g!(grid::DeviceGrid{T1, T2}, mpts::DeviceParticle{T1, T2}, G::T2) where {T1, T2}
    ...
    if ix ≤ mpts.np
        ...
        @KAunroll for iy in 1:mpts.NIC
            ...
            @Σ grid.fs[p2n, 3] += -Ω * (∂Nz * (σzz + σw) + ∂Nx * σzx + ∂Ny * σyz) + Nij * mmG
        end
    end
end
...
@kernel function tg2p!(grid::DeviceGrid{T1, T2}, mpts::DeviceParticle{T1, T2}) where {T1, T2}
    ix = @index(Global)
    if ix ≤ mpts.np
...
function tprocedure!(conf::Config, grid::DeviceGrid{T1, T2}, mpts::DeviceParticle{T1, T2}) where {T1, T2}
    ...
    while t_cur < t_tol
        ...
        tg2p!(dev)(ndrange=dev_mpts.np, dev_grid, dev_mpts)
        pb!(printer, t_cur, Δt)
        t_cur += Δt
        h5.iters[] += 1
    end
