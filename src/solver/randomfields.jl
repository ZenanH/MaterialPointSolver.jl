...
"""
    grf_gc!(dev, Yf, pts, rl, rr; precision=Float64, If=2.0, k_m=100.0, Nh=5000, 
        sf=1)
...
```
"""
function grf_gc!(dev, Yf, pts, rl, rr;
    precision = Float64,
    If        = 2.0,
    k_m       = 100.0,
    Nh        = 5000,
    ...
    c_pts = Array(T2.(pts))
    c_Yf  = Array(T2.(Yf))
    d_pts = host2device(dev, c_pts)
    d_Yf  = host2device(dev, c_Yf)
    @info "data is ready on $(dev)"
    for _ = 1:Nh
        ...
        a = randn(T2) 
        b = randn(T2)
        compute1!(dev)(ndrange=length(d_Yf), d_Yf, a, b, V1, V2, V3, d_pts)
    end
    compute2!(dev)(ndrange=length(d_Yf), d_Yf, C)
    max_val = reduce(max, d_Yf)
    min_val = reduce(min, d_Yf)
    compute3!(dev)(ndrange=length(d_Yf), d_Yf, max_val, min_val, rl, rr)
    Yf .= Array(d_Yf)
end

"""
    grf_ec!(dev, Yf, pts, rl, rr; precision=Float64, If=[10.0, 8.0, 5.0], Nh=5000, 
        sf=1)
...
```
"""
function grf_ec!(dev, Yf, pts, rl, rr; 
    precision = Float64,
    If        = [10.0, 8.0, 5.0],
    Nh        = 5000,
    sf        = 1
